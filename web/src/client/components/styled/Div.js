...
export default class Div extends React.Component {
  ...
  render() {
    ...
    let mediaNumber
    let mediaProperty
    Object.keys(props)/*.sort()*/.forEach(property => {
      if (property in document.body.style)
        css += `${property}: ${props[property]};\n`
      else if ((mediaIndex = property.indexOf('_')) && mediaIndex>-1) {
        ...
        if (mediaProperty in document.body.style) {
          if (medias[mediaNumber] === undefined)
            medias[mediaNumber] = ''
          medias[mediaNumber] += `${mediaProperty}: ${props[property]};\n`
        }
      }
    })
    ...
    return <Theme css={css} {...props}>{props.children}</Theme>
  }
}
